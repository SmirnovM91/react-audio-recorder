(function(){"use strict";try{if(typeof document!="undefined"){var e=document.createElement("style");e.appendChild(document.createTextNode(".audio-recorder{background-color:#ebebeb;box-shadow:0 2px 5px #bebebe;border-radius:20px;width:40px;display:flex;align-items:center;transition:all .2s ease-in}.audio-recorder-mic{color:#000;padding:0;width:40px;height:40px}.audio-recorder .audio-recorder-mic{border-radius:20px}.audio-recorder.recording .audio-recorder-mic{border-radius:0}.audio-recorder-timer,.audio-recorder-status{margin-left:10px;font-family:Segoe UI,Tahoma,Geneva,Verdana,sans-serif;font-size:14px;font-weight:400;line-height:1}.audio-recorder-status{margin-left:15px;display:flex;align-items:baseline;flex-grow:1;animation-name:fading-ar-status;animation-duration:2s;animation-iteration-count:infinite}.audio-recorder-status-dot{background-color:#d00;border-radius:50%;height:10px;width:9px;margin-right:5px}.audio-recorder-options{height:16px;cursor:pointer;padding:12px 6px 12px 12px}.audio-recorder-options~.audio-recorder-options{padding:12px 12px 12px 6px;border-radius:0 5px 5px 0}.recording{border-radius:12px;width:300px;transition:all .2s ease-out}.display-none{display:none}@keyframes fading-ar-status{0%{opacity:1}50%{opacity:0}to{opacity:1}}.audio-recorder-mic-button{width:50px;height:50px;border-radius:50%;background-color:#ebebeb;cursor:pointer;padding:15px;display:flex;align-items:center;justify-content:center}")),document.head.appendChild(e)}}catch(r){console.error("vite-plugin-css-injected-by-js",r)}})();
import d, { useState as u, useRef as v, useCallback as k, useEffect as A } from "react";
const b = () => {
  const [w, g] = u(!1), [M, j] = u(!1), [m, I] = u(0), i = v(), [c, L] = u(), [z, D] = u(), n = () => {
    const t = setInterval(() => {
      I((e) => e + 1);
    }, 1e3);
    L(t);
  }, C = () => {
    c != null && clearInterval(c), L(void 0);
  };
  return {
    startRecording: k(() => {
      c == null && navigator.mediaDevices.getUserMedia({ audio: !0 }).then((t) => {
        var N, o, a;
        g(!0);
        const e = [];
        i.current = new MediaRecorder(t), (N = i.current) == null || N.addEventListener("dataavailable", (r) => {
          r.data.size > 0 && e.push(r.data);
        }), (o = i.current) == null || o.addEventListener("stop", () => {
          var r, T;
          console.log(e, (r = i.current) == null ? void 0 : r.mimeType), D(
            new Blob(e, { type: (T = i.current) == null ? void 0 : T.mimeType })
          ), i.current = null;
        }), (a = i.current) == null || a.start(), n();
      }).catch((t) => console.log(t));
    }, [c]),
    stopRecording: () => {
      var t, e;
      (t = i == null ? void 0 : i.current) == null || t.stop(), (e = i == null ? void 0 : i.current) == null || e.stream.getTracks().forEach((N) => N.stop()), C(), I(0), g(!1), j(!1);
    },
    togglePauseResume: () => {
      var t, e;
      M ? (j(!1), (t = i == null ? void 0 : i.current) == null || t.resume(), n()) : (j(!0), C(), (e = i == null ? void 0 : i.current) == null || e.pause());
    },
    recordingBlob: z,
    isRecording: w,
    isPaused: M,
    recordingTime: m
  };
}, l = "data:image/svg+xml;base64,", Y = "data:image/svg+xml;base64,", O = "data:image/svg+xml;base64,", x = "data:image/svg+xml;base64,";
const Z = ({
  onRecordingComplete: w,
  recorderControls: g,
  classes: M
}) => {
  var y, S, s, t, e, N, o, a;
  const {
    startRecording: j,
    stopRecording: m,
    togglePauseResume: I,
    recordingBlob: i,
    isRecording: c,
    isPaused: L,
    recordingTime: z
  } = g != null ? g : b(), [D, n] = u(!1), C = (r = !0) => {
    n(r), m();
  };
  return A(() => {
    (D || g) && i != null && w != null && w(i);
  }, [i]), c ? /* @__PURE__ */ d.createElement("div", {
    className: `audio-recorder ${c ? "recording" : ""} ${(s = M == null ? void 0 : M.AudioRecorderClass) != null ? s : ""}`,
    "data-testid": "audio_recorder"
  }, c ? /* @__PURE__ */ d.createElement("span", {
    className: `audio-recorder-status ${c ? "" : "display-none"} ${(t = M == null ? void 0 : M.AudioRecorderStatusClass) != null ? t : ""}`
  }, /* @__PURE__ */ d.createElement("span", {
    className: "audio-recorder-status-dot"
  }), "Recording") : /* @__PURE__ */ d.createElement("img", {
    src: l,
    className: `audio-recorder-mic ${(e = M == null ? void 0 : M.AudioRecorderStartSaveClass) != null ? e : ""}`,
    onClick: j,
    "data-testid": "ar_mic",
    title: "start recording"
  }), /* @__PURE__ */ d.createElement("span", {
    className: `audio-recorder-timer ${c ? "" : "display-none"} ${(N = M == null ? void 0 : M.AudioRecorderTimerClass) != null ? N : ""}`,
    "data-testid": "ar_timer"
  }, Math.floor(z / 60), ":", String(z % 60).padStart(2, "0")), /* @__PURE__ */ d.createElement("img", {
    src: L ? O : Y,
    className: `audio-recorder-options ${c ? "" : "display-none"} ${(o = M == null ? void 0 : M.AudioRecorderPauseResumeClass) != null ? o : ""}`,
    onClick: I,
    title: L ? "Resume recording" : "Pause recording",
    "data-testid": "ar_pause"
  }), /* @__PURE__ */ d.createElement("img", {
    src: x,
    className: `audio-recorder-options ${c ? "" : "display-none"} ${(a = M == null ? void 0 : M.AudioRecorderDiscardClass) != null ? a : ""}`,
    onClick: () => C(!0),
    title: "Discard Recording",
    "data-testid": "ar_cancel"
  })) : /* @__PURE__ */ d.createElement("div", {
    className: `audio-recorder-mic-button ${(y = M == null ? void 0 : M.AudioRecorderClass) != null ? y : ""}`,
    onClick: j
  }, /* @__PURE__ */ d.createElement("img", {
    src: l,
    className: `audio-recorder-mic ${(S = M == null ? void 0 : M.AudioRecorderStartSaveClass) != null ? S : ""}`,
    "data-testid": "ar_mic",
    title: "start recording"
  }));
};
export {
  Z as AudioRecorder,
  b as useAudioRecorder
};
